...
interface PendingSaleListProps {
  sales: Sale[];
...
export const PendingSaleList: React.FC<PendingSaleListProps> = ({ sales, onConfirm, onEdit, onDelete }) => {
  if (sales.length === 0) {
    return (
      <div className="bg-white p-8 rounded-md shadow-sm border border-slate-200/80 text-center text-slate-500">
        <h3 className="text-lg font-semibold">No hay ventas pendientes</h3>
        <p className="mt-2">Todas tus ventas están al día. ¡Buen trabajo!</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-md shadow-sm border border-slate-200/80 overflow-hidden">
        <div className="overflow-x-auto">
            <table className="w-full text-sm text-left text-slate-500 responsive-table">
                ...
                <tr>
                    <th scope="col" className="px-6 py-4">Fecha (Estimada)</th>
                    ...
                    <th scope="col" className="px-6 py-4 text-right">Ganancia Potencial</th>
                ...
                {sales.map((sale) => (
                    ...
                    <td data-label="Fecha (Est.)" className="px-6 py-5 font-medium text-slate-900 whitespace-nowrap">{formatDate(sale.date)}</td>
                    ...
                    <td data-label="Monto" className="px-6 py-5 text-right">{formatCurrency(sale.amount)}</td>
                    <td data-label="Comisión (%)" className="px-6 py-5 text-right">{sale.commissionRate}%</td>
                    <td data-label="Ganancia Potencial" className="px-6 py-5 text-right font-semibold text-amber-600">{formatCurrency(sale.amount * sale.commissionRate / 100)}</td>
                    <td className="px-6 py-5 actions-cell">
                        <div className="flex items-center justify-center space-x-4 md:justify-center">
                        <button onClick={() => onConfirm(sale)} className="text-slate-500 hover:text-emerald-600 transition-colors" title="Confirmar Venta">
                            <CheckCircleIcon />
                        </button>
